...
- Added
  ...
  - Add Activation sheet in Solution input and Solution.\_get_Activation() function. The Activation matrix can be used to control the on/off switching of a Tank for a Reaction, e.g. to make sure that the Aeration Reaction only takes place in the Aeration Tank.
