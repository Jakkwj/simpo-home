...
load_dotenv(".env")
QINGYUN_API_KEY = environ.get("QINGYUN_API_KEY_AUTO")
# QINGYUN_API_KEY = environ.get("QINGYUN_API_KEY_GEMINI")
...
def get_content(model: str, result: dict[str, str], access: str, is_en: bool = True) -> str:
    """根据 is_en 创建中英文内容"""
    return f"""---
        slug: /{result["DOI"]}
...
def main(
    ...
    if result:
        ...
        content = get_content(answer_model, result, access)
        with open(target_dir / f"{result_title_}.mdx", "w") as file:  # type: ignore
            file.write(content)

        content = get_content(answer_model, result, access, is_en=False)
