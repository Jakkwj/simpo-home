...
The major achievement of this version is **pH Calculation**.

- Added

  - Added support for pH calculation.
  - Added support for Min/Max functions in BioModel Rate.
  - Simulation/Sensitivity/Uncertainty/Estimation figure results can now be exported as SVG format.
  - SVG format for Band/Density/Skeleton/PCD/VariableNSE will be saved locally, and PNG format will be removed.

- Fixed

  - Now the Calculation Time will be the same as the DataSet Measured Time.
  - Check if the Client Project Dir exists before deleting it.

- Changed
  - Remove Improved Euler (Forward Euler is good eought).
  - Use msg
