# 2026

<!--
## 2025-09-24

版本**0.12.0**发布。

该版本的主要成果是 **pH 计算**。

- 添加

  - 添加 pH 计算的支持。
  - 添加 在 BioModel Rate 中的 Min/Max 函数的支持。
  - 现在可以将 Simulation/Sensitivity/Uncertainty/Estimation 的图表结果导出为 SVG 格式。
  - Band/Density/Skeleton/PCD/VariableNSE 的 SVG 格式将保存在本地，PNG 格式将被移除。

- 修复

  - 现在 Calculation Time 将与 DataSet Measured Time 相同。
  - 删除 Client Project Dir 前先进行确认是否存在。

- 更改
  - 删除 Improved Euler （Forward Euler 已经足够）。 -->

<!-- ## 2026-02-16

版本**1.0.2**发布。

此版本是一次小的升级和稳定。

- **更改**
  - DataSet 的 pickle 中的 Hydraulics_Partical 需要改成 Hydraulics_Particulate

- **修正**
  - 去掉 `Kernel` 中 `Grid_dt_second == 0.0D0` 这个无法进一步计算的判断条件（因为 `Grid_dt_second` 可能正好落在一个实测点上，导致 `remaining_times` 为 0），从而避免`Setp Tolerance`的设置可能导致计算无法正确进行。 -->

  <!-- - 添加 API key -->
  <!-- - Unc 支持收入初始值 -->


## 2026-09-13

版本**1.3.8**发布。

此版本是一次小的升级和稳定。

- **新增**
  - 注册时需要填写单位等额外信息。

- **修正**
  - **BioModel.Rate** 中的 **If** 函数的判断条件现在可以正确提取公因数。
  - **BioModel**/**DataSet**/**Project** 修改内容的同时修改命名现在可以正确地保存修改内容。
  - **DataSet** 现在可以准确导出/导出超大布局。
  - **Estimation** 的画图和结果保存时，现在会准确地显示转换后的时间单位。
  - 修复注册时有时邮箱无法收到注册确认邮件的问题。
  - 修复 **Inflow**/**Flow** 的时间轴不相同时的计算问题。
  - 修复 **BioModel** 保存时溶解性物质没有放到前面的问题。
  - 修复表格无法用 ctrl/shift 进行多选的问题。

- **改进**
  - 支持修改用户名。
  - 修改密码/邮箱合并到 Account 页面。

---


## 2026-07-26

版本**1.3.7**发布。

此版本是一次小的升级和稳定。

- **修正**
  - **Uncertainty**/**Estimation** 计算时的参数的时间单位现在可以正确显示在结果文件和图片中。
  - **BioModel** 中输入的 **Component** 会自动把溶解性物质（**S\_** 开头）放到最前。
  - 修复某些情况会会重复添加 **pH** 到 **Inflow** 中的问题。

- **改进**
  - 优化了多子图的画图结果，现在子图每行最多四列。
  - 动态画图现在支持缩放（ctrl+鼠标滚轮缩放，ctrl+0恢复原缩放）。
  - **DataSet** 的 **Inflow** 初始值与 **Solution** 中对应的值不同时会写入日志 Warning。

---

## 2026-06-07

版本**1.3.6**发布。

此版本是一次小的升级和稳定。

- **修正**
  - **NEGATIVE ERROR** 时将正确地推送到前端。
  - **NEGATIVE ERROR** 时将弹出本地`log`文件。
  - **NEGATIVE ERROR** 在 **Uncertainty**/**Estimation** 计算时将会自动剔除负数结果（而不是现在的停止计算）。
  - 修正 **Measured** 有空值时的 **NSE** 计算公式。
  - 修正了在 **Project** 页面保存后左侧菜单栏错误的问题。
  - 修正了 **Oxygen** 的单位判断问题。
  - 修正了在 **DataSet** 中修改 **Tank** 的名称和位置时导致的 **Weight** 和 **Activator** 错位的问题。

- **改进**
  - **DataSet** 保存时将自动把 **BioCalculated** 为 `False` 的 **Tank** 放到最后，同时这个顺序将同步到 **DataSet.Measured** 和 **Solution.Variable**，**Solution.Weight** 和 **Solution.Activator**。
  - 现在将通过分块的方式保存结果 Excel 文件，避免 Stack Overflow 错误 （Windows 平台）。
  - 现在保存结果 Excel 文件时，如果 sheetname 太长（超过31长度），将减少长度并生成一张名称映射表（**SheetMap**）。

---

## 2026-05-24

版本**1.3.5**发布。

此版本是一次小的升级和稳定。

- **修正**
  - 修复 **RFGA** 无法评估 **Flow** 的错误。
  - 修正了包含 **Flow** 的 **Evaluation** 计算，现在将统一进行体积换算。
  - **Inflow** 将在 **Figure** 中正确显示。

- **改进**
  - 导入文件新建 **DataSet**/**Solution** 时，将对错误格式进行提醒。

---

## 2026-05-22

版本**1.3.4**发布。

此版本是一次小的升级和稳定。

- **修正**
  - 修正画图进程，防止画图丢包。
  - **Console** 中 **STOP** 可以正确关闭计算进程。

- **改进**
  - 对冗余 **Component** 的计算进行了进一步优化，现在只有在接近 0 时会放开变步长限制（而不是原来的一直放开）。
  - 对变浓度计算时的步长进行了优化，降低每一个变浓度跨越点的计算误差。
  - **Resolution Rate** 现在支持放大原采样点，可用于方便地更精确的获得计算采样结果。

---

## 2026-05-14

版本**1.3.3**发布。

此版本是一次小的升级和稳定。

- **修正**
  - 修正了一些已知错误。

- **改进**
  - **Version** 现在支持排序

---

## 2026-05-10

版本**1.3.2**发布。

此版本是一次小的升级和稳定。

- **修正**
  - 拼写错误 `Aera` 修正为 `Area`。
  - `Pump` 计算现在可以正确触发。

- **改进**
  - 进一步提升关闭 `Real-Time Plotting` 选项时的计算速度。

---

## 2026-05-08

版本**1.3.1**发布。

此版本是一次小的升级和稳定。

- **修正**
  - `Simulation` 计算引擎现在可以正确地进行动态画图。
  - 修正点沉淀池变体积的计算公式。

---

## 2026-05-05

版本**1.3.0**发布。

该版本的主要成果包括：

<div class="theme-admonition theme-admonition-info admonition_node_modules-@docusaurus-theme-classic-lib-theme-Admonition-Layout-styles-module alert alert--info">
  <div class="admonitionContent_node_modules-@docusaurus-theme-classic-lib-theme-Admonition-Layout-styles-module">
  - `Python` 版本从 `3.12.12` 升级到 `3.13.13`。
  - `Estimation` 计算引擎新增 `RFGA` 算法。
  - `Solution` 现在支持 pickle 缓存。
  - 性能大幅优化。
  </div>
</div>

<!-- LHS/GA 等多线程计算提高最多90% -->

- **新增**
  - `Estimation` 计算引擎新增 `RFGA` 算法。
  - `BioModel.Parameter`/`Solution.Variable` 的 `Value` 和 `LowerBound` 现在支持负数。
  - `Solution` 现在支持 pickle 缓存。

- **改进**
  - 线程大幅优化:
    - Simulation 方程求解
    - Sensitivity/Uncertainty/Estimation 并行优化
    - 动态绘图
    - 生成结果文件
    - 生成运行代码

- **修正**
  - 修正变体积，即 `Constant Volume` 为 `False` 时的计算公式，现在可以用 `CAS` 工艺正确模拟 `SBR` 工艺。
  - 修正了一些已知错误。

---

## 2026-04-09

版本**1.2.0**发布。

该版本的主要成果包括：

<div class="theme-admonition theme-admonition-info admonition_node_modules-@docusaurus-theme-classic-lib-theme-Admonition-Layout-styles-module alert alert--info">
  <div class="admonitionContent_node_modules-@docusaurus-theme-classic-lib-theme-Admonition-Layout-styles-module">
  - 新增 **Steady State** 计算功能。
  </div>
</div>

- **新增**
  - 支持稳态计算。
  - `BioModel.Matrix.Rate` 的计算将自动提取公因式，进一步提高运算速度。

- **修正**
  - 修正`get_result_Sim()` 函数中的 `dynamic_volume` 的保存错误。

---

## 2026-04-06

版本**1.1.1**发布。

此版本是一次小的升级和稳定。

- **新增**
  - `BioModel.Matrix.Rate` 支持 `If()`, `Log()`, `Ln()` 和 `Exp()` 函数。

- **改进**
  - `BioMatrix.get_BioStoichoimetric()` 函数现在会对 `stoichiometric` 统一赋初始值 `0.0D0`, 计算速度提高。
  - 当一个 `Pump` 连接到了一个 `Tank` 时，没有在 `DataSet.Target` 标注 `DO` 将发出提醒
  - 当一个 `Pump` 连接到了一个 `Tank` 时，没有在 `Solution.Conversion` 与 `Target` 中的 `DO` 有转化关系，将发出提醒
  - `Version` 中的 **Description** 现在支持 **Markdown** 格式
  - `Version` 现在支持打印 PDF 文件导出
  - `DataSet` 自动布局算法优化。

- **修正**
  - `client.post_result()` 函数中 `payload["result"]` 中添加了无穷大 `inf` 的处理，暂时用文本 `"inf"` 代替。

---

## 2026-03-26

版本**1.1.0**发布。

该版本的主要成果包括：

<div class="theme-admonition theme-admonition-info admonition_node_modules-@docusaurus-theme-classic-lib-theme-Admonition-Layout-styles-module alert alert--info">
  <div class="admonitionContent_node_modules-@docusaurus-theme-classic-lib-theme-Admonition-Layout-styles-module">
  - 新增 **Pump** 模块。
  - 支持在 **DataSet** 直接进行 **曝气** 计算。
  </div>
</div>

- **新增**
  - 支持 **曝气** 计算，相关参数在 `DataSet.Pump` 中设置。
  - `DataSet.Unit` 新增了 `Area` 和 `Height` 的单位支持。
  - `DataSet.Target` 中支持指定 `Oxygen`。
  - `DataSet.Tank` 中新增了 4 个参数：`DiffuserNumber`，`DiffuserDepth`，`AlphaFactor` 和 `BetaFactor`。
  - `DataSet.FlowChart` 的交互界面新增 `Pump` 模块，用于支持 **曝气** 计算。
  - `Pump` 模块支持使用曝气风机的 `SOTE-流量` 曲线进行插值拟合。
  - 新建 `DataSet` 时，新增 4 种预设 `Layout`：`CAS-Pump`, `AO-Pump`, `AAO-Pump` 和 `UCT-Pump`。

- **改进**
  - 优化了 `Best Variable` 页面的 `SetToValue` 的交互和提示。
  - 优化了 `Rank Variable` 页面的 `SetToEvaluation` 的交互和提示。
  - 在 `Detail` 页面中点击 `Edit` 按钮改名后，路由将跳转至改名后的新路径。
  - 通过 AI 新建 BioModel 时，如果 Public 中已有此模型，会发起提醒，并提供一个快捷跳转按钮用于方便克隆。

- **修正**
  - 修正了 `BioModelDetail` schema 的 `composition` 和 `ionization` 为空时的判断。
  - 修正了 `_get_empty_variable_row` 函数的 `initial_value` 的赋值方式。
  - 现在点击 BioModel 的 Public 页面的 `CLASSIC` 按钮后可以正确显示模型总数量。
  - 函数 `cleanInvisibleChars` 现在能在 Matrix/Compositions/Connections/Conversions 页面中输入公式保存时正确去除看不见的空格。

---

## 2026-03-01

版本**1.0.2**发布。

此版本是一次小的升级和稳定。

- **添加**
  - BioModel 的 Public 页面添加`Classic`按钮，点击后只显示经典模型。
  - 添加函数 `cleanInvisibleChars`, 用于在 Matrix/Compositions/Connections/Conversions 页面中输入公式保存时，会去除更多看不见的空格。
  - 使用 **AI** 新建 **BioModel** 时，可选择模型数量增加到 6 个， 包括 Gemini 3.1 Pro/Gemini 3.0 Pro/Gemini 2.5 Flash/Claude 4.6 Sonnet/Claude 4.5 Sonnet/Claude 3.5 Haiku。

- **更改**
  - DataSet 的 pickle 中的 Hydraulics_Partical 需要改成 Hydraulics_Particulate。
  - 源代码中的 `Partical` 统一改成 `Particulate`。

- **改进**
  - Matrix/Compositions/Connections/Conversions 页面中输入公式后的零宽空格被去除，即移除 createTextNode('\u200B')。
  - 优化使用 **AI** 新建 **BioModel** 时的提示词，使得解析论文更加容易。
  - 改进了空白新建 `BioModel` 的内容，现在是一个完整的通过平衡验证的矩阵，空白新建 `Project` 时可以直接计算得出结果。
  - 改进了空白新建 `DataSet`, 现在选择 `Targets` 时会添加 Tank 内第二行 Measured 数据, 从而使得空白新建 `Project` 时可以直接计算 `Uncertainty` 和 `Estimation`。

- **修正**
  - 修复左侧菜单栏缩小时，鼠标左键和中键盘点击`Quick Start`/`Changelog`/`Resource`无法跳转的问题。
  - 修复了 Library 中 Components 和 Parameters 的查询错误。
  - 现在 Calculaiton Console 可以正确的获取客户端电脑的 CPU 核心数量了（os.cpus().length 改为 navigator.hardwareConcurrency）。
  - `Setp Tolerance` 现在最多只支持2位小数点。
  - `Setp Tolerance` 在计算 Report 中将固定显示为2位小数点。
  - 更新了 AQUASIM T2P17 的计算案例。
  - 修复了 BioModel/DataSet/Project 的 Personal 表格中点击 Cloned 表头时排序的错误。
  - `figure_data_stream_body()` 补充了 Inflow 只有一个 Component 时的处理。
  - 修复了空白新建 `DataSet` 时的问题, 现在新建 `Layout` 为 `Batch` 且同时选择 `Targets` 将不会新建一个空白的 `Inflow`。
  - 尝试修复 `The garbage collector is trying to clean up non-checked-in connection <AdaptedConnection <asyncpg.connection.Connection object at 0x7f80f4b40e60>>, which will be terminated.  Please ensure that SQLAlchemy pooled connections are returned to the pool explicitly, either by calling ``close()`` or by using appropriate context managers to manage their lifecycle.`
  - `_get_redundant_component_no()` 补充了只有一个 `redundant` 时的处理。
  - 去掉 `Kernel` 中 `Grid_dt_second == 0.0D0` 这个无法进一步计算的判断条件（因为 `Grid_dt_second` 可能正好落在一个实测点上，导致 `remaining_times` 为 0），从而避免 `Setp Tolerance` 的设置可能导致计算无法正确进行。
  - 修正了一些已知错误。

---

## 2026-02-01

版本**1.0.1**发布。

此版本是一次小的升级和稳定。

- **修正**
  - 当画图函数遇到`BrokenProcessPool`错误时, 会使用主进程继续画图。
  - 修复了`BioModel`偶然出现的保存错误（该错误由前端的不可见空格引起）。
  - 修正了一些已知错误。

---

## 2026-01-19

版本**1.0.0**发布。

该版本的主要成果包括：

<div class="theme-admonition theme-admonition-info admonition_node_modules-@docusaurus-theme-classic-lib-theme-Admonition-Layout-styles-module alert alert--info">
  <div class="admonitionContent_node_modules-@docusaurus-theme-classic-lib-theme-Admonition-Layout-styles-module">
  - 支持 **Gemini 3** 创建 `BioModel`。
  - 支持 **pH** 计算。
  - `Python` 版本从 `3.11.9` 升级到 `3.12.12`。
  - 后端框架从 `fastapi` 迁移至 `litestar`。
  - 网址从 [https://www.simpowater.org]() 迁移至 [https://www.simpowater.com]()，访问旧网址将自动重定向至新网址。
  </div>
</div>

- **新增**
  - 支持通过 **AI** 自动解析 **SCI Paper PDF** 的方式（**Gemini 3**）新建 `BioModel`。
  - 支持 **pH** 计算，相关参数在`BioModel.Ionization`中设置。
  - 计算结果现在支持 **svg** 图片格式下载。
  - `Public` 和 `Private` 现在不需要登录就可以访问。
  - 本地动态画图窗口支持用鼠标/键盘左右键切换图片，切换时图片将保持宽高比。

- **更改**
  - **Improved Euler** 将不再支持。通过降低 **Forward Euler** 的 **Step Tolerance** 可以（更快地）达到同样的计算精度。

- **改进**
  - 客户端下载体积大幅减少。
  - 优化本地动态画图功能，减少内存和 CPU 消耗。
  - 前端页面优化，更多地支持自适应屏幕。
  - `Edit` 和 `Export` 按钮从 `Profile` 页面移动到 `Detail` 页面。
  - 可以在 `Project.Detail` 页面对 `BioModel` 和 `DataSet` 进行 `Edit` 和 `Export`。
  - 优化 `BioModel.Matrix`, `Project.Conversion` 等矩阵中的公式输入。
  - `Dashboard` 左侧菜单支持折叠，刷新页面将自动保存折叠状态。
  - 本地保存的 **svg** 图片将移除坐标轴上的科学计数法。
  - 禁用 **Backspace** 按键的回退功能, 防止公式输入未保存前意外退出页面。
  - 顶部弹窗通知内容现在不会重叠显示。
  - `Project.Calculation`的画图结果可以进行精细控制并保存状态，包括：
    - 坐标轴的范围设置。
    - 线的宽度和符号的大小。
    - 画图结果的宽度。
    - 图例的位置。

- **修正**
  - `Project.Calculation.Console` 现在可以正确地获取 `log`。
  - `Project.Targets` 现在可以正确保存。
  - `Inflow` 图现在将正确显示。
  - `BioModel`/`DataSet`/`Project` 现在能被正确地删除。
  - 修复本地保存的 **svg** 图片最后一个点有时无法正确画出的错误。
  - 动态画图窗口可以正确的刷新。
  - 动态画图窗口在任务栏的 icon 现在可以正确显示。
  - 修正了一些已知错误。
